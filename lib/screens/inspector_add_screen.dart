...
class InspectorAddScreen extends StatefulWidget {
  const InspectorAddScreen({Key? key}) : super(key: key);

  @override
  State<InspectorAddScreen> createState() => _InspectorAddState();
}

class _InspectorAddState extends State<InspectorAddScreen> {
  ...
  void initState() {
    birthDateCtl.text = "";
    genderCtl.text = "Hombre";
    rolCtl.text = "ADMINISTRADOR";
    super.initState();
  }
  ...
  Future<void> addInspector() async {
    ...
    if (newInspector.name == "" ||
      ...
      dynamic res = await _inspectorService.addInspector(newInspector);
      if (context.mounted) {
        if (res["status"] == 200) {
